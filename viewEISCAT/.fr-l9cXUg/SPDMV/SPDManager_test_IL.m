...
if isempty(varargin)
  ...
  datasetinfo.projname='uFPI';
  
  % Set parameters to be fetched (refer to initial files)
  datasetinfo.paralist={'IL','IU','IE'};
  datasetinfo.save=1; % 0 - no save; 1 - save
  
  % Set plotting pannels
  drawopt.order={[1,-1],[2,-2],[3,-3],[3,-3;1,-1;2,-2]};
...
function [datelist]=getdatelist(id)
  ...
  if id==0
    ...
    ed_dn=datenum(dv);
  elseif id==1
    st_dn=datenum([2012 12 01 18 00 00]);
    ed_dn=datenum([2012 12 01 22 00 00]);
  elseif id==2
